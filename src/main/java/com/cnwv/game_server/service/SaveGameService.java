...
public class SaveGameService {
    ...
    public SaveGameResponse get(Long userId) {
        PlayerSave save = saveRepo.findById(userId)
                .orElseThrow(() -> new NoSuchElementException("Save not found"));
    ...
    /** 최초 생성(이미 있으면 409) */
    @Transactional
    public SaveGameResponse create(Long userId, SaveGameRequest req) {
        if (saveRepo.existsById(userId)) {
            throw new IllegalStateException("Save already exists");
        }
        ...
        s.setTutorialClear(nvl(req.getTutorialClear(), true)); // 예시 기본값 유지
    ...
    /** 전체 저장(업서트: 없으면 생성) */
    @Transactional
    public SaveGameResponse upsert(Long userId, SaveGameRequest req) {
        ...
        if (req.getVersion() != null && req.getVersion() != save.getVersion()) {
